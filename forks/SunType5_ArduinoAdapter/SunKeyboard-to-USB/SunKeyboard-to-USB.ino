...
void setup() {
    Serial.begin(115200);   // Normal serial for Serial Monitor Debugging
...
void loop() {
    ...
    if (c != 0xFFFFFFFF) {
        switch(c) {
            case 45: // Decr Vol Key to Enable Click Sound
                sunSerial.write(CMD_DISABLE_CLICK);
                break;
            case 2: // Mute Key to Disable Click Sound
                sunSerial.write(CMD_ENABLE_CLICK);
                break;
            case 4: // Incr Vol to Enable Bell
                sunSerial.write(CMD_ENABLE_BELL);
                break;
            case 48: // Power Key to Disable Bell 
                sunSerial.write(CMD_DISABLE_BELL);
                break;
            case 98: // Ignore NumLock
                break;
            case 119: // Caps Lock Toggle
                if (!CAPS_LOCK_ON) {
                    CAPS_LOCK_ON = true;
                    led_cmd[1] += 8;
