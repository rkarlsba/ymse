...
def main():
    ...
    try:
        ...
        # Get all users (50)
        user = json.dumps(zapi.user.get(output='extend', selectUsrgrps="extend",
                                        selectRole="extend", selectMedias="extend",
                                        selectMediatypes="extend"), indent=4)
        with open(f'{outputdir}/user.load.json', 'w') as f:
            print(user, file=f)

        # Get all usergroups (52)
        usergroup = json.dumps(zapi.usergroup.get(output='extend', selectTagFilters='extend',
                                                  selectUsers='extend', selectHostGroupRights='extend',
                                                  selectTemplateGroupRights='extend'), indent=4)
