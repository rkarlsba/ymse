    ...
    for app in page:
        ...
        for field_name in ["keyCredentials", "passwordCredentials"]:
            ...
            for val in field:
                ...
                if nyeste_daysleft is None or nyeste_daysleft < daysleft:
                    nyeste_daysleft = daysleft
                    nyeste_appinfo = appinfo
                if sec2days(timespan(startDateTime,endDateTime)) > 740:
                    lange_timespans.append(appinfo)
            
            if nyeste_appinfo:
                antall_credentials += 1
                appdates.append(nyeste_appinfo)
                if nyeste_daysleft <= warndays and nyeste_daysleft > -10:
                    warnlist.append(nyeste_appinfo)
                if nyeste_daysleft > 0:
                    expirelist.append(nyeste_appinfo)


nextexpire=min([int(remainingdays(d.get("endDateTime"))) for d in expirelist])
status = f"apps={antall_apps} newestcredentials={antall_credentials} allecredentials={antall_allecredentials} nextexpire={nextexpire}"
...
if len(warnlist) > 0:
    ...
    print(f"WARNING: {nextexpire} days left - {len(warnlist)} azure application{flere} access will expire within {warndays} days, first in {nextexpire} days: {liste} status {status}")
if len(warnlist) == 0:
